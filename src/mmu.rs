use crate::common::{get_bit, get_bit_as_bool, set_bit, HWReg, LCDC, SCX};
use crate::cpu::Z80;
...
impl MMU {
    ...
    pub(crate) fn get_stack_16(&self) -> &[u8] {
        &self.data[0xFFF0..0xFFFE]
    }
    ...
    pub fn write8(&mut self, addr: u16, val: u8) {
        ...
        if addr < 0x8000 {
            ...
            if addr >= 0x4000 && addr <= 0x5FFF {
                info!(
                    "writing to RAM Bank Number or Upper bits of ROM bank number {:04x}",
                    val
                );
                return;
            }
            ...
            println!(
                "trying to write outside of RW memory {:04x} at addr {:04x}",
                val, addr
            );
            panic!("halting");
        }
        ...
        if addr == SC_REGISTER {
            ...
            if val == 0x81 {
                info!(
                    ...
                    self.hardware.SB,
                    char::from_u32(self.hardware.SB as u32).unwrap()
                );
            }
            // panic!("halting");
            return;
        ...
        if addr == OBP0_ADDR {
            self.hardware.OBP0 = val;
            return;
        }
        if addr == OBP1_ADDR {
            self.hardware.OBP1 = val;
            return;
        }
        if addr == WX_ADDR {
            self.hardware.WX = val;
            return;
        }
        if addr == WY_ADDR {
            self.hardware.WY = val;
            return;
        }
        if addr == SCX_SCROLL_X {
        ...
        let arr = [
            NR10_SOUND, NR11_SOUND, NR12_SOUND, NR13_SOUND, NR14_SOUND, NR21_SOUND, NR22_SOUND,
            NR23_SOUND, NR24_SOUND, NR30_SOUND, NR31_SOUND, NR32_SOUND, NR33_SOUND, NR34_SOUND,
            NR42_SOUND, NR44_SOUND, NR50_SOUND, NR51_SOUND, NR52_SOUND,
        ];
        ...
        if addr >= 0xFF00 && addr < 0xFF80 {
            info!(
                "trying to write to the registers area {:04x} {:02}",
                addr, val
            );
            return;
        }
