            ...
            if let Some(event) = self.context.event_pump().unwrap().poll_event() {
                match event {
                    ...
                    Event::KeyDown {
                        ...
                        to_cpu.send(Press(JoyPadKey::A));
                    }
                    Event::KeyUp {
                        keycode: Some(Keycode::Z),
                        ..
                    } => {
                        to_cpu.send(Release(JoyPadKey::A));
                    }
                    Event::KeyDown {
                    ...
                    Event::KeyUp {
                        ...
                        to_cpu.send(Release(JoyPadKey::B));
                    }
                    Event::KeyDown {
                        keycode: Some(Keycode::Space),
                        ..
                    } => {
                        to_cpu.send(Press(JoyPadKey::Select));
                    }
                    Event::KeyUp {
                        keycode: Some(Keycode::Space),
                        ..
                    } => {
                        to_cpu.send(Release(JoyPadKey::Select));
                    }
                    Event::KeyDown {
                        keycode: Some(Keycode::Return),
                        ..
                    } => {
                        to_cpu.send(Press(JoyPadKey::Start));
                    }
                    Event::KeyUp {
                        keycode: Some(Keycode::Return),
                        ..
                    } => {
                        to_cpu.send(Release(JoyPadKey::Start));
                    }
                    Event::KeyDown {
                        keycode: Some(Keycode::Left),
                        ..
                    } => {
                        to_cpu.send(Press(JoyPadKey::Left));
                    }
                    Event::KeyUp {
                        keycode: Some(Keycode::Left),
                        ..
                    } => {
                        to_cpu.send(Release(JoyPadKey::Left));
                    }
                    Event::KeyDown {
                        keycode: Some(Keycode::Right),
                        ..
                    } => {
                        to_cpu.send(Press(JoyPadKey::Right));
                    }
                    Event::KeyUp {
                        keycode: Some(Keycode::Right),
                        ..
                    } => {
                        to_cpu.send(Release(JoyPadKey::Right));
                    }
                    Event::KeyDown {
                        keycode: Some(Keycode::Up),
                        ..
                    } => {
                        to_cpu.send(Press(JoyPadKey::Up));
                    }
                    Event::KeyUp {
                        keycode: Some(Keycode::Up),
                        ..
                    } => {
                        to_cpu.send(Release(JoyPadKey::Up));
                    }
                    Event::KeyDown {
                        keycode: Some(Keycode::Down),
                        ..
                    } => {
                        to_cpu.send(Press(JoyPadKey::Down));
                    }
                    Event::KeyUp {
                        keycode: Some(Keycode::Down),
                        ..
                    } => {
                        to_cpu.send(Release(JoyPadKey::Down));
                    }
                    Event::KeyDown {
                        keycode: Some(Keycode::V),
                        ..
                    } => {
                        to_cpu.send(JumpNextVBlank());
                    }
                    Event::KeyUp {
                        keycode: Some(Keycode::V),
                        ..
...
impl Screen {
    pub fn init(settings: &ScreenSettings, sss: Arc<Mutex<SSS>>) -> Screen {
        ...
        let tex = canvas
            .texture_creator()
            .create_texture(PixelFormatEnum::ARGB8888, TextureAccess::Target, 256, 256)
            .unwrap();
        let tex2 = canvas
            .texture_creator()
            .create_texture(PixelFormatEnum::ARGB8888, TextureAccess::Target, 128, 256)
            .unwrap();
        Screen {
            context: sdl_context,
            canvas,
